import {type TokenBalance, getInstructionDescriptor} from '@belopash/solana-stream'
...
export function handleWhirlpool(ins: Instruction, block: Block): SolanaSwapTransfer {
    ...
    ] = getInnerTransfersByLevel(ins, block.instructions, 1).map((t) => tokenProgram.instructions.transfer.decode(t))
    ...
    const swapPrice = tokenIn && tokenOut && swapEvent ? getPoolPrice(swapEvent, tokenIn, tokenOut) : null
...
function getPoolPrice(swapEvent: Traded, tokenIn: TokenBalance, tokenOut: TokenBalance) {
...
function getSwapEvent(ins: Instruction, block: Block): Traded | null {
    ...
    if (logs.length > 1 && !logs[1].message.startsWith('fee_growth:')) {
...
function getTokenReserves(
    ...
    const tokenBalances = getInstructionBalances(ins, block)
    const tokenA = tokenBalances.find((b: TokenBalance) => b.account === tokenAVault)
    const tokenB = tokenBalances.find((b: TokenBalance) => b.account === tokenBVault)
