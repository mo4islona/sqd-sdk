import type {TokenBalance} from '@belopash/solana-stream'
import type {SolanaSwapTransfer} from '../../solana-swap-stream'
import {type Block, type Instruction, getInstructionBalances, sortAccounts} from '../../utils'
...
export class RaydiumCpmmSwapBaseInputHandler extends RaydiumCpmmSwapBaseHandler {
    ...
    handleSwap(): SolanaSwapTransfer {
        const {inputVault, outputVault, inputTokenMint, outputTokenMint, poolAddress} = this.getAccounts()

        const [token0Mint, token1Mint] = sortAccounts(inputTokenMint, outputTokenMint)
        ...
        const {inputTokenAmount, outputTokenAmount, authority, owner, tokenInAccount, tokenOutAccount} =
        ...
        const slippage = this.getSlippageSwapBaseInput(inputTokenAmount, outputTokenAmount, token0, token1)

        const instructionTokenBalances = getInstructionBalances(this.instruction, this.block)
        ...
        return {
            ...
            in: {
                ...
                token: instructionTokenBalances.find((b: TokenBalance) => b.account === tokenInAccount),
            },
            out: {
                amount: outputTokenAmount.amount,
                token: instructionTokenBalances.find((b: TokenBalance) => b.account === tokenOutAccount),
            },
            slippage,
            reserves: {
                tokenA: token0,
    ...
    private getAmountOut(amountIn: bigint, reserveIn: bigint, reserveOut: bigint): number {
        const amountInWithFee =
            amountIn * (RaydiumCpmmSwapBaseHandler.FEE_DENOMINATOR - RaydiumCpmmSwapBaseHandler.PROTOCOL_FEE)
