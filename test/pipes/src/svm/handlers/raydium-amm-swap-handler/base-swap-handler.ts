...
import {type Block, type Instruction, getDecodedInnerTransfers, getInstructionBalances} from '../../utils'
...
export abstract class RaydiumCpmmSwapBaseHandler {
    ...
    protected getAccounts(): {
        ...
        const {
            accounts: {inputTokenMint, outputTokenMint, inputVault, outputVault, poolState: poolAddress},
    ...
    protected getTokenData(): {inputToken: Token; outputToken: Token} {
        ...
        const inputVaultTokenBalance = tokenBalances.find((tb: any) => tb.account === inputVault)
        const outputVaultTokenBalance = tokenBalances.find((tb: any) => tb.account === outputVault)
    ...
    protected getTokenReserves({
        ...
        const {preAmount: preSwapInputTokenReserves, postDecimals: inputTokenDecimals} = inputVaultTokenBalance

        const {preAmount: preSwapOutputTokenReserves, postDecimals: outputTokenDecimals} = outputVaultTokenBalance

        const isToken0Input = inputTokenMint === token0Mint
        const isToken1Input = inputTokenMint === token1Mint

        const token0Reserves = {
            amount: isToken0Input ? BigInt(preSwapInputTokenReserves) : BigInt(preSwapOutputTokenReserves),
            decimals: isToken0Input ? inputTokenDecimals : outputTokenDecimals,
        }

        const token1Reserves = {
            amount: isToken1Input ? BigInt(preSwapInputTokenReserves) : BigInt(preSwapOutputTokenReserves),
