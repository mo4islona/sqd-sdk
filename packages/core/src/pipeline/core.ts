 ...
 */
export type DataDuplex<In, Out, Query, Cursor> = DataTarget<Cursor, In, Query, DataSource<Cursor, Out, Query>>

export type DataPassThrough<TCursor, TValue, TQuery> = DataDuplex<TValue, TValue, TQuery, TCursor>
    ...
    pipe<TReturn>(
        ...
        const target =
            typeof targetOrFactory === 'function'
                ? targetOrFactory({ unfinalized: this.#unfinalized })
                : targetOrFactory
