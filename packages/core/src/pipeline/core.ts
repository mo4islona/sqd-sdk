...
export interface DataSource<TCursor, TValue, TQuery> {
   ...
   */
  [Symbol.asyncIterator](opts?: DataReadRequest<TCursor, TQuery>): AsyncIterableIterator<TValue>;
}
...
export interface DataSourceConstructor {
  new <TCursor, TValue, TQuery>(config: DataSourceConfig<TCursor, TValue, TQuery>): DataSource<TCursor, TValue, TQuery>;
}
  ...
  implements DataSource<TCursor, TValue, TQuery>
{
      ...
      read: async function* (readOpts: DataReadRequest<TCursor, TQuery>) {
        ...
        for await (const message of self.read(readOpts)) {
          ...
          const unfinalizedIndex = mergedData.findIndex(
            (item: DataItem<TCursor, TValue>) => cursorUtils.compare(item.cursor, message.finalizedHead!).isGreater,
          );
  ...
  async reduce<UValue>(reducer: (accumulator: UValue, value: TValue) => UValue, initialValue: UValue): Promise<UValue> {
    let accumulator = initialValue;
