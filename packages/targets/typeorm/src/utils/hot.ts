...
export class ChangeTracker {
    ...
    constructor(private em: EntityManager, private statusSchema: string, private blockHeight: number) {
        this.statusSchema = this.escape(this.statusSchema)
    }

    trackInsert(type: EntityTarget<EntityLiteral>, entities: EntityLiteral[]): Promise<void> {
    ...
    async trackUpsert(type: EntityTarget<EntityLiteral>, entities: EntityLiteral[]): Promise<void> {
        let meta = this.getEntityMetadata(type)

        let touchedRows = await this.fetchEntities(
            meta,
            entities.map((e) => e.id)
        ).then((entities) => new Map(entities.map(({id, ...fields}) => [id, fields])))
    ...
    async trackDelete(target: EntityTarget<EntityLiteral>, ids: string[]): Promise<void> {
        let meta = this.getEntityMetadata(target)
        let deletedEntities = await this.fetchEntities(meta, ids)
        return this.writeChangeRows(
            deletedEntities.map((e) => {
                let {id, ...fields} = e
                return {
                    kind: 'delete',
                    table: meta.tableName,
    ...
    private async fetchEntities(meta: EntityMetadata, ids: string[]): Promise<EntityLiteral[]> {
        let entities = await this.em.query(`SELECT * FROM ${this.escape(meta.tableName)} WHERE id = ANY($1::text[])`, [
            ids,
        ])
    ...
    private getEntityMetadata(target: EntityTarget<EntityLiteral>): EntityMetadata {
        return this.em.connection.getMetadata(target)
    }

    private escape(name: string): string {
        return escape_(this.em, name)
    }
}

export async function rollbackBlock(statusSchema: string, em: EntityManager, blockHeight: number): Promise<void> {
    let schema = escape_(em, statusSchema)
        ...
        switch (rec.change.kind) {
            ...
            case 'update': {
                let setPairs = Object.keys(rec.change.fields).map((column, idx) => {
                    return `${escape_(em, column)} = $${idx + 1}`
                })
                if (setPairs.length) {
                    await em.query(`UPDATE ${table} SET ${setPairs.join(', ')} WHERE id = $${setPairs.length + 1}`, [
                        ...Object.values(rec.change.fields),
                        id,
                    ])
                }
                break
            ...
            case 'delete': {
                let columns = ['id', ...Object.keys(rec.change.fields)].map((col) => escape_(em, col))
                let values = columns.map((col, idx) => `$${idx + 1}`)
                await em.query(`INSERT INTO ${table} (${columns}) VALUES (${values.join(', ')})`, [
                    id,
                    ...Object.values(rec.change.fields),
                ])
                break
...
function escape_(em: EntityManager, name: string): string {
